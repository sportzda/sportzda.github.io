...
test.describe('Platform-Specific Authentication Storage', () => {
    ...
    test.describe('Login and Storage', () => {
        test('should store authentication token after successful login', async ({ page }) => {
            ...
            // Verify token is stored (in sessionStorage or localStorage depending on platform)
    ...
    test.describe('Logout Functionality', () => {
        test('should clear token from storage on logout', async ({ page }) => {
            ...
            // Mock index.html route to confirm navigation attempt
            let navigatedToIndex = false;
            await page.route('**/index.html', async (route) => {
                navigatedToIndex = true;
                await route.fulfill({
                    status: 200,
            ...
            // Click logout button
            const clickPromise = page.click('#logoutBtn');
            ...
            // Verify navigation happened (which means logout executed)
